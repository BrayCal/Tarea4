...
$$
El error asociado con la aproximación *está ligado a la cantidad de veces que hagamos la aproximación*, es decir, al número de pasos en el tiempo que utilicemos en nuestra solución.
$\sum\epsilon = \sum_{k=0}^{N-1}\frac{h^2}{2}\left. \frac{d^2x}{dt^2} \right|{x_k, t_k} = \frac{h}{2}\sum{k=0}^{N-1}h\left.\frac{df}{dt}\right|_{x_k, t_k}\\
...
El método se deriva aplicando la serie de Taylor alrededor del punto medio $t + h/2$ para obtener el valor de la función en el punto $x(t + h).

Similarmente, podemos hacer lo mismo para $x(t)$.
Finalmente,
$$
